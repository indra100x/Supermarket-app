...
class TCPListenCommand extends Command
{
    ...
    protected $description = 'Start a TCP server to receive supply requests and stock updates';
    ...
    public function handle()
    {
        ...
        while ($client = stream_socket_accept($server)) {
            ...
            $response = match ($request['action']) {
                'supply_stock' => $this->handleSupplyRequest($request),
                'receive_stock' => $this->handleReceiveStock($request),
    ...
    private function handleSupplyRequest($request)
    {
        ...
        // Notify the receiving supermarket
        $data = [
